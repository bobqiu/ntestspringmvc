				...
				switch (e.which)
				{
					...
					default:
						if ($('.aw-invite-dropdown')[0])
						{
							...
							$.get(G_BASE_URL + '/search?type=users&q=' + encodeURIComponent($(this).val().substring(flag, ti).replace('@', '')) + '&limit=' + limit, function (result)
							{
								if ($('.aw-invite-dropdown')[0])
								{
									if (result.length != 0)
...
AWS.User =
{
	...
	{
		$.post(G_BASE_URL + '/question/ajax/thanks/', 'questionId=' + question_id, function (result)
		{
			if (result.errno != 1)
			{
				AWS.alert(result.err);
...
AWS.Dropdown =
{
	...
	*/
	get_dropdown_list: function(selector, type, data)
	{
		...
		{
			case 'search' :
				url = G_BASE_URL + '/search?q=' + encodeURIComponent(data) + '&limit=5';
			break;

			case 'publish' :
				url = G_BASE_URL + '/search?type=questions&q=' + encodeURIComponent(data) + '&limit=5';
			break;

			case 'redirect' :
				url = G_BASE_URL + '/search?q=' + encodeURIComponent(data) + '&type=questions&limit=30&is_question_id=1';
			break;

			case 'invite' :
			case 'inbox' :
				url = G_BASE_URL + '/search?type=users&q=' + encodeURIComponent(data) + '&limit=10';
			break;

			case 'topic_question' :
				url = G_BASE_URL + '/search?type=questions,articles&q=' + encodeURIComponent(data) + '&topic_ids=' + CONTENTS_RELATED_TOPIC_IDS + '&limit=50';
			break;

			case 'topic' :
				url = G_BASE_URL + '/search?type=topics&q=' + encodeURIComponent(data) + '&limit=10';
			break;

			case 'questions' :
				url = G_BASE_URL + '/search?type=questions&q=' + encodeURIComponent(data) + '&limit=10';
			break;

			case 'articles' :
				url = G_BASE_URL + '/search?type=articles&q=' + encodeURIComponent(data) + '&limit=10';
			break;
...
AWS.Message =
{
	...
	check_notifications: function()
	{
		...
		$.get(G_BASE_URL + '/home/notifications/', function (result)
		{
			...
			else
			{
				...
				switch ($(this).attr('data-type'))
				{
					case 'question':
						...
						var comment_form_action = G_BASE_URL + '/question/' + $(this).attr('data-id')+'/comment/save';
						var comment_data_url = G_BASE_URL + '/question/' + $(this).attr('data-id')+'/comments/list' ;
						break;
