$(function()
{
	...
	// 初始化编辑器/publish/upload/(question|article)/([0-9]+)/([0-9]+)/([0-9]+)
    ...
    if (ATTACH_ACCESS_KEY != '' && $('.aw-upload-box').length)
    {
		...
		var fileupload = new FileUpload('file', '.aw-editor-box .aw-upload-box .btn', '.aw-editor-box .aw-upload-box .upload-container', G_BASE_URL + '/publish/upload/' + PUBLISH_TYPE + '/' + ATTACH_ACCESS_KEY, {
				'editor' : editor
			});
    }
